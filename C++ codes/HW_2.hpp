...
class MyMatrix{
    ...
    MyMatrix(int rows_, int columns_){
        rows =rows_;
        columns=columns_;
    ...
    MyMatrix(const MyMatrix & copy_from){
        rows=copy_from.rows;
    ...
    MyMatrix & operator=(const MyMatrix& copy_from){
...
MyMatrix operator*(double a, const MyMatrix & A){
    MyMatrix C(A.rows, A.columns);
    for (int i = 0; i < A.rows; i++)
    {
        for (int j = 0; j < A.columns; j++)
...
class MyVector: public MyMatrix{
    MyVector(int rows_): MyMatrix(rows_, 1){}
};
